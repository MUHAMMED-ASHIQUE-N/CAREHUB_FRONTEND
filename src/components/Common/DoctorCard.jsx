...
const DoctorCard = ({id, name, speciality, image  }) => {
  ...
  return (
    <div onClick={()=>navigate(`/doctor-profile/${id}`)}  ref={ref} 
    className={`flex flex-col  gap-4  bg-white  rounded-lg shadow-lg border border-gray-100 cursor-pointer  transform duration-1000  ease-out delay-100 transition-all   ${inView ?  "opacity-100 translate-y-0" :  "opacity-0 translate-y-32"}`}
    key={id}> 
